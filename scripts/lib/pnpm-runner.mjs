/**
 * The pnpm entry every profile package operation goes through — the Muse
 * desktop's installer and the community market alike, because both reach pnpm
 * by name and the packaged shim in `.desktop-bin` points here.
 *
 * Ported from DSH Desktop (`vendors/dsh-desktop/packages/dsh-desktop-market-installer/pnpm-runner.mjs`)
 * with the log marker and knob env vars renamed; behavior kept identical.
 *
...
export const IDLE_TIMEOUT_MS =
  Number(process.env.MUSE_PNPM_IDLE_TIMEOUT_MS ?? process.env.DSH_DESKTOP_PNPM_IDLE_TIMEOUT_MS) || 300_000
...
export const STALL_AFTER_FAILURE_MS =
  Number(process.env.MUSE_PNPM_FAILURE_STALL_MS ?? process.env.DSH_DESKTOP_PNPM_FAILURE_STALL_MS) || 20_000
/** Prefix of every line this runner contributes to a package operation's output. */
export const MARKER = 'muse pnpm runner:'
