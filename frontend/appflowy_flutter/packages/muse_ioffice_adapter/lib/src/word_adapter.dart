...
final class MuseIofficeWordAdapter implements MuseEngineAdapter {
  ...
  MuseEngineAdapterManifestV1 get manifest => MuseEngineAdapterManifestV1(
      ...
      engine: const MuseEngineIdentityV1(
          vendor: 'ioffice', engineId: 'word', engineVersion: '0.1.0'),
      placements: const [MusePlacementKind.desktopLocal],
      platforms: const [MusePlatformV1(os: 'macos', arch: 'arm64')],
      formats: const [
        MuseFormatSupportV1(formatId: 'ooxml.word', mimeTypes: [
  ...
  Future<MuseProbeResultV1> probe(
    ...
    cancellation.throwIfCancelled();
    final correctFormat = request.descriptor.format.formatId == 'ooxml.word' &&
    ...
    final base = admission.engineBound &&
    ...
    return MuseProbeResultV1(
        ...
        materializationKinds:
            available ? const [MuseMaterializationKind.bytesHandle] : const [],
  ...
  Future<MuseEngineSessionHandleV1> open(MuseOpenEngineSessionRequestV1 request,
    ...
    if (request.materialization.kind != MuseMaterializationKind.bytesHandle)
      throw const MuseAdapterException('BYTES_HANDLE_REQUIRED',
          retryable: false);
