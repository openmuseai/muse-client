...
void main() {
  test('current admission is honest: Word view only, placeholders unavailable',
      () {
    final catalog = MuseOfficeAdmissionCatalog.current();
    expect(catalog[MuseOfficeKind.word].effectiveModes, {MuseEngineMode.view});
  ...
  test('manifest does not advertise edit/commit before toDocx certification',
      () {
  ...
  test('probe validates artifact, ABI, layout, format and platform', () async {
    expect(
        ...
        isTrue);
    expect(
        (await _adapter(health: _health(digestMatches: false))
                .probe(_probe(), MuseCancellationToken()))
            .available,
        isFalse);
    expect(
        (await _adapter().probe(
                _probe(format: 'ooxml.spreadsheet'), MuseCancellationToken()))
            .available,
        isFalse);
    expect(
        (await _adapter().probe(_probe(os: 'windows'), MuseCancellationToken()))
            .available,
        isFalse);
  });
  test('heap edit remains internal ephemeral and never becomes save edit',
      () async {
    final adapter = _adapter(ephemeral: true);
    final result = await adapter.probe(
        _probe(mode: MuseRequestedMode.preferEdit), MuseCancellationToken());
    ...
    MuseProbeRequestV1(
        ...
        descriptor: MuseProbeDescriptorV1(
            ...
            format: MuseResourceFormat(
                formatId: format, confidence: MuseFormatConfidence.verified),
            sizeBytes: 42,
