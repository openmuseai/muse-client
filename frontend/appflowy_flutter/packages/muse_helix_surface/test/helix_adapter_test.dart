...
void main() {
  test('current admission remains closed until all H0 gates pass', () async {
    ...
    expect(_adapter(MuseHelixAdmission.current).manifest.modes,
        [MuseEngineMode.view],
        reason: 'manifest is a candidate; runtime probe remains unavailable');
    expect(
        (await _adapter(MuseHelixAdmission.current)
                .probe(_probe(), MuseCancellationToken()))
            .available,
        isFalse);
  });
  test('read-only certification never advertises edit or commit', () async {
    ...
    expect((await adapter.probe(_probe(), MuseCancellationToken())).available,
        isTrue);
    expect(
        (await adapter.probe(
                _probe(mode: MuseRequestedMode.edit), MuseCancellationToken()))
            .available,
        isFalse);
  });
  test('probe rejects binary content, bad digest and uncertified platform',
      () async {
    final admission = _admission(view: true);
    expect(
        ...
        isFalse);
    expect(
        (await _adapter(admission, health: _health(digest: false))
                .probe(_probe(), MuseCancellationToken()))
            .available,
        isFalse);
    expect(
        (await _adapter(admission)
                .probe(_probe(os: 'windows'), MuseCancellationToken()))
            .available,
        isFalse);
  });
  test('PTY input requires a user grant scoped to session generation',
      () async {
    ...
    await adapter.resize(
        handle, const MuseHelixPtySize(columns: 160, rows: 50));
    ...
    final other = MuseEngineSessionHandleV1(
        ...
        generation: 2,
        state: handle.state);
    expect(() => adapter.sendUserInput(other, [1], grant),
  ...
  test('working-copy state prevents dirty close and models conflict recovery',
      () {
    ...
    state.complete(retry, MuseWorkingCopyCommitResult.committed,
        newRevision: 'r2');
...
MuseHelixAdapter _adapter(MuseHelixAdmission admission,
        {MuseHelixRuntimeHealth? health, _Runtime? runtime}) =>
    MuseHelixAdapter(
...
MuseOpenEngineSessionRequestV1 _open() => const MuseOpenEngineSessionRequestV1(
    ...
    surface: MuseOpenSurfaceV1(
        windowRef: 'w', placement: 'main', reuse: 'compatible'),
    generation: 1);
...
final class _Runtime implements MuseHelixRuntime {
  ...
  @override
  Future<MuseHelixRuntimeSession> spawn(MuseOpenEngineSessionRequestV1 request,
          MuseCancellationToken cancellation) async =>
      session;
}
...
final class _Session implements MuseHelixRuntimeSession {
  ...
  Future<void> sendUserInput(
      List<int> bytes, MuseHelixUserInputGrant grant) async {
    inputCalls++;
  }
