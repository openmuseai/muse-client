...
final class MuseHelixAdapter implements MuseEngineAdapter {
  ...
  MuseEngineAdapterManifestV1 get manifest => MuseEngineAdapterManifestV1(
      ...
      engine: const MuseEngineIdentityV1(
          vendor: 'helix', engineId: 'hx', engineVersion: '25.7.1'),
  ...
  Future<MuseProbeResultV1> probe(
    ...
    cancellation.throwIfCancelled();
    final platform = admission.platforms.any((item) =>
        item.os == request.placement.os && item.arch == request.placement.arch);
    final format = {'text.plain', 'code.source'}
    ...
    return MuseProbeResultV1(
        ...
        materializationKinds:
            available ? const [MuseMaterializationKind.workingCopy] : const [],
  ...
  Future<MuseEngineSessionHandleV1> open(MuseOpenEngineSessionRequestV1 request,
    ...
    if (request.materialization.kind != MuseMaterializationKind.workingCopy)
      throw const MuseAdapterException('WORKING_COPY_REQUIRED',
          retryable: false);
  ...
  MuseHelixUserInputGrant grantUserInput(MuseEngineSessionHandleV1 session) {
    ...
    return MuseHelixUserInputGrant.issueForUser(
        session.sessionRef, session.generation);
  }

  Future<void> sendUserInput(MuseEngineSessionHandleV1 session, List<int> bytes,
      MuseHelixUserInputGrant grant) async {
    if (grant.sessionRef != session.sessionRef ||
        grant.generation != session.generation)
      throw const MuseAdapterException('INPUT_AUTHORITY_MISMATCH',
          retryable: false);
    await _sessions[session.sessionRef]?.sendUserInput(bytes, grant);
  }

  Future<void> resize(
      MuseEngineSessionHandleV1 session, MuseHelixPtySize size) async {
