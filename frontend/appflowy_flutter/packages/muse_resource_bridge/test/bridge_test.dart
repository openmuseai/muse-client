...
void main() {
  test('request/status/cancel share one neutral Host Bridge envelope', () {
    ...
    for (final value in values) {
      ...
      expect(MusePresentationBridgeEnvelope.fromJson(value.toJson()).toJson(),
          value.toJson());
  ...
  test('web grants require exact audience, HTTPS origin and live TTL', () {
    ...
    MuseRemoteMaterializationGrant grant(
        ...
        MuseRemoteMaterializationGrant(
            ...
            generation: 1);
    broker.admit(grant());
    expect(broker.authorize('h1', resourceRef: 'r1', generation: 1).revision,
        'rev1');
    broker.revoke('h1');
    expect(broker.activeGrantCount, 0);
    expect(() => broker.admit(grant(audience: 'other')),
        throwsA(isA<MuseDataPlaneException>()));
    expect(() => broker.admit(grant(origin: 'http://viewer.muse.test')),
        throwsA(isA<MuseDataPlaneException>()));
    now = 3000;
  ...
  test('mobile profile is view-only and enforces low-memory size ceiling', () {
    expect(
        MuseClientCapabilityProfile.mobile.allowedModes, {MuseEngineMode.view});
    final broker = MuseDataPlaneBroker(
  ...
  test('generation revoke invalidates all remote reads', () {
    ...
    expect(() => broker.authorize('loop', resourceRef: 'r', generation: 7),
        throwsA(isA<MuseDataPlaneException>()));
  });
}
