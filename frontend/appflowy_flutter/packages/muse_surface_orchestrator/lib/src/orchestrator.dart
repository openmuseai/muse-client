...
final class MuseSurfaceOrchestrator {
  ...
  Future<void> closeSession(String sessionRef,
      {String reason = 'user-close'}) async {
  ...
  Future<void> dispose() async {
    ...
    for (final record in _records.values
        .where((item) => item.status.phase != MusePresentationPhase.terminal)) {
      record.cancellation.cancel();
      _commit(
          record,
          _failure(record,
  ...
  Future<void> _run(_RequestRecord record) async {
    ...
    try {
      ...
      final descriptor =
          await _resources.describe(request.resourceRef, record.cancellation);
      record.cancellation.throwIfCancelled();
      ...
      while (remaining.isNotEmpty) {
        ...
        final decision = MuseRouteDecisionV2(
            ...
            effectiveMode:
                MuseEffectiveMode.values.byName(selection.effectiveMode.name),
        ...
        try {
          ...
          final handle = await selection.registration.adapter
              .open(open, record.cancellation);
          Future<void> close() => selection.registration.adapter
              .close(handle, reason: 'open-saga-rollback');
          saga.own(close);
          record.cancellation.throwIfCancelled();
          ...
          lastAttemptError = error is MuseAdapterException
              ? error.code
              : 'ADAPTER_OPEN_FAILED';
          remaining.removeWhere((outcome) =>
              outcome.registration.key == selection.registration.key);
  ...
  void _phase(_RequestRecord record, MusePresentationPhase phase,
      {String? attemptRef}) {
  ...
  MusePresentationFailureReceiptV2 _failure(_RequestRecord record,
      ...
      MusePresentationFailureReceiptV2(
          ...
          errorCode: errorCode,
          retryable: retryable);
  MusePresentationSuccessReceiptV2 _success(_RequestRecord record,
      ...
      MusePresentationSuccessReceiptV2(
          ...
          warnings: warnings);
  void _validateHandle(MuseOpenEngineSessionRequestV1 open, String adapterRef,
    ...
    if (handle.adapterRef != adapterRef ||
      ...
      throw const MuseAdapterException('SESSION_OWNERSHIP_MISMATCH',
          retryable: false);
    if (!{
