...
final class MuseFakeResourceGateway implements MuseResourceGateway {
  ...
  Future<MuseResourceDescriptorV1> describe(
      String resourceRef, MuseCancellationToken cancellation) async {
    describeCalls++;
  ...
  Future<MuseResourceMaterializationV1> materialize(
      ...
      required MuseCancellationToken cancellation}) async {
    materializeCalls++;
  ...
  Future<void> _step(
      MuseFakeFailurePoint point, MuseCancellationToken cancellation) async {
...
final class MuseFakeEngineAdapter implements MuseEngineAdapter {
  ...
  Future<MuseEngineSessionHandleV1> open(MuseOpenEngineSessionRequestV1 request,
    ...
    openCalls++;
    await _step(MuseFakeFailurePoint.open, cancellation,
  ...
  Future<void> _step(
      MuseFakeFailurePoint point, MuseCancellationToken cancellation,
      {bool checkAfterDelay = true}) async {
    cancellation.throwIfCancelled();
    if (delay > Duration.zero) await Future<void>.delayed(delay);
