...
void main() {
  test('100 duplicate opens share one operation, surface, and terminal receipt',
      () async {
    final fixture = _Fixture();
    final request = fixture.request('preq.duplicate');
    final operations =
        List.generate(100, (_) => fixture.orchestrator.request(request));
    expect(operations.toSet(), hasLength(1));
    final receipts =
        await Future.wait(operations.map((operation) => operation.terminal));
    ...
    expect(terminalStatus.decision!.selectedAdapterRef,
  ...
  test('requestRef cannot be reused for a different request', () {
    ...
    expect(
        () => fixture.orchestrator
            .request(fixture.request('same', resourceRef: 'resource.other')),
        throwsA(isA<MuseOrchestratorException>()));
  });

  test('explicit focus reuses the owned surface without materializing again',
      () async {
    final fixture = _Fixture();
    final opened = await fixture.orchestrator
        .request(fixture.request('preq.open'))
        .terminal as MusePresentationSuccessReceiptV2;
    final focusedRequest = MusePresentationRequestV2(
        ...
        requestedAt: 2);
    final focused = await fixture.orchestrator.request(focusedRequest).terminal
  ...
  test('cancel wins atomically and late ready generation is disposed',
      () async {
    final fixture = _Fixture(delay: const Duration(milliseconds: 20));
    final operation =
        fixture.orchestrator.request(fixture.request('preq.cancel'));
    ...
    expect(
        fixture.orchestrator.status('preq.cancel')!.receipt, same(cancelled));
    expect(fixture.adapter.surfaces, isEmpty);
    expect(fixture.resources.liveHandles, isEmpty);
    expect(fixture.orchestrator.activeSessionCount, 0);
  ...
  test(
      ...
      () async {
    final resources = MuseFakeResourceGateway(descriptor: _Fixture.descriptor);
    ...
    final orchestrator =
        MuseSurfaceOrchestrator(resources: resources, registry: registry);
    final receipt =
        await orchestrator.request(_Fixture.makeRequest('preq.late')).terminal;
  ...
  test('failed primary rolls back and opens fallback with visible receipt',
      () async {
    final resources = MuseFakeResourceGateway(descriptor: _Fixture.descriptor);
    ...
    expect(resources.liveHandles, hasLength(1),
  ...
  test('1000 open-close soak returns all resource and surface counters to zero',
      () async {
    final fixture = _Fixture();
    for (var index = 0; index < 1000; index++) {
      final receipt = await fixture.orchestrator
          .request(fixture.request('preq.soak.$index',
              resourceRef: 'resource.docx.1'))
          .terminal as MusePresentationSuccessReceiptV2;
  ...
  test('feature rollout, breaker, latency and leak gates fail closed', () {
    final rollout = MuseEngineRolloutGate(adapterModes: {
    ...
    expect(
        rollout.decide('helix~1', actorRef: 'actor.allowed').mayOpen, isTrue);
    ...
    const MuseLeakSnapshot(
            pendingRequests: 0, sessions: 0, materializations: 0, surfaces: 0)
        .verifyZero();
    expect(
        () => const MuseLeakSnapshot(
                pendingRequests: 0,
  ...
  test('neutral Host Bridge request/status drives fake Surface end to end',
      () async {
    final fixture = _Fixture();
    ...
    MusePresentationBridgeEnvelope envelope(
        ...
        MusePresentationBridgeEnvelope(
            ...
            actorRef: 'actor.1',
            payload: payload);
    final accepted = await service.handle(envelope(
        'wire.1',
    ...
    await fixture.orchestrator.request(fixture.request('preq.bridge')).terminal;
    final terminal = await service.handle(envelope(
        'wire.2',
    ...
    expect(terminal.decision!.selectedAdapterRef,
        fixture.adapter.manifest.adapterRef);
    await fixture.orchestrator.closeSession(
...
final class _Fixture {
  _Fixture({Duration delay = Duration.zero})
      : resources =
            MuseFakeResourceGateway(descriptor: descriptor, delay: delay),
        adapter = MuseFakeEngineAdapter(
            manifest: museFakeManifest(adapterId: 'fake.word'), delay: delay) {
  ...
  static const descriptor = MuseResourceDescriptorV1(
      ...
      format: MuseResourceFormat(
          formatId: 'ooxml.word', confidence: MuseFormatConfidence.verified),
      capabilities: [
  ...
  MusePresentationRequestV2 request(String ref,
          {String resourceRef = 'resource.docx.1'}) =>
      makeRequest(ref, resourceRef: resourceRef);
  static MusePresentationRequestV2 makeRequest(String ref,
          {String resourceRef = 'resource.docx.1'}) =>
