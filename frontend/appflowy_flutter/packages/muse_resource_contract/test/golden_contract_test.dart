...
void main() {
  group('authoritative wire fixtures', () {
    _fixtureSuite(
      '../../../../../../middlewares/dsh/core/contract-resource/fixtures/v1/messages.json',
      MuseResourceWireValueV1.fromJson,
    );
    _fixtureSuite(
      '../../../../../../middlewares/dsh/core/contract-presentation/fixtures/v2/messages.json',
  ...
  test('schema digests remain pinned to the F0 contract freeze', () async {
    ...
    for (final entry in expected.entries) {
      ...
      final path =
          '../../../../../../middlewares/dsh/core/contract-${entry.key}/fixtures/$version/schema-digests.json';
...
void _fixtureSuite(String path, _Parser parser) {
  test('round-trips valid and rejects invalid: $path', () async {
    final fixtures =
        (jsonDecode(await File(path).readAsString()) as List).cast<Map>();
    for (final raw in fixtures) {
      ...
      if (fixture['valid'] == true) {
        ...
        expect(() => parser(kind, value), throwsA(isA<FormatException>()),
