...
List<T> enumList<T extends Enum>(
  ...
  final result = objectList(map[field], field).map((raw) {
    if (raw is! String)
...
List<String> stringList(Map<String, Object?> map, String field,
    {bool unique = true}) {
  final result = objectList(map[field], field).map((value) {
    if (value is! String || value.isEmpty) {
      throw MuseContractFormatException(
