...
void main() {
  test('Wave 1 manifest exposes only certified text/image/PDF view', () {
    ...
    expect(adapter.manifest.formats.map((e) => e.formatId),
        ['document.pdf', 'image.raster', 'text.plain']);
    expect(adapter.manifest.formats.map((e) => e.formatId),
  ...
  test('probe rejects active content, edit and implicit network runtime',
      () async {
    final adapter = _adapter();
    final token = MuseCancellationToken();
    expect(
        (await adapter.probe(_probe(active: MuseActiveContent.none), token))
            .available,
        isTrue);
    expect(
        (await adapter.probe(_probe(active: MuseActiveContent.present), token))
            .available,
        isFalse);
    expect(
        (await adapter.probe(
                _probe(
                    active: MuseActiveContent.none,
                    mode: MuseRequestedMode.edit),
                token))
            .available,
        isFalse);
    final network = _adapter(
        health: const MuseViewerRuntimeHealth(
            ...
            runtimeDigest: 'bad'));
    expect(
        (await network.probe(_probe(active: MuseActiveContent.none), token))
            .available,
        isFalse);
  });
  test(
    ...
    final bridge = MuseViewerBridgeSession(
        origin: 'https://viewer.muse.test', nonce: 'n1', generation: 2);
    ...
    expect(bridge.receive(sourceOrigin: 'https://evil.test', message: message),
        isNull);
    expect(
        bridge.receive(
    ...
    expect(
        bridge
            .receive(
                sourceOrigin: 'https://viewer.muse.test', message: message)!
            .type,
        'viewer.ready');
    bridge.close();
    expect(
...
final class _Runtime implements MuseViewerRuntime {
  ...
  @override
  Future<MuseViewerRuntimeSession> open(MuseOpenEngineSessionRequestV1 request,
