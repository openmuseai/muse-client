...
final class MuseOpenFileViewerAdapter implements MuseEngineAdapter {
  ...
  MuseEngineAdapterManifestV1 get manifest => MuseEngineAdapterManifestV1(
      ...
      platforms: [
        MusePlatformV1(os: _os(profile.end), arch: _arch(profile.end))
      ],
      formats: catalog.effective
          .map((item) => MuseFormatSupportV1(
  ...
  Future<MuseProbeResultV1> probe(
    ...
    final allowed = certification != null &&
  ...
  Future<MuseEngineSessionHandleV1> open(MuseOpenEngineSessionRequestV1 request,
    ...
    if (!manifest.materializations.contains(request.materialization.kind))
      throw const MuseAdapterException('MATERIALIZATION_NOT_SUPPORTED',
          retryable: false);
