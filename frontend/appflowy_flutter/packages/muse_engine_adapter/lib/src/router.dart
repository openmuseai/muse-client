...
final class MuseEngineRouter {
  ...
  MuseRouteSelection? select(
    ...
    for (final outcome in outcomes) {
      ...
      if (probe == null ||
        ...
        ranked.add(_Ranked(outcome: outcome, mode: null, score: 0, reasons: [
          if (policy.deniedAdapterRefs
              .contains(outcome.registration.adapterRef))
            'POLICY_DENIED'
          else
      ...
      final mode =
          _mode(probe.effectiveModes, requestedMode, policy.allowModeDowngrade);
      if (mode == null) {
        ranked.add(_Ranked(
            outcome: outcome,
            mode: null,
      ...
      final preference =
          policy.preferredAdapterRefs.indexOf(outcome.registration.adapterRef);
      if (preference >= 0) score += 200 - preference;
      final reasons = <String>[
        if (_isDowngrade(requestedMode, mode)) 'MODE_DOWNGRADED'
      ];
      ranked.add(_Ranked(
          outcome: outcome, mode: mode, score: score, reasons: reasons));
    }
    ranked.sort((a, b) {
      final score = b.score.compareTo(a.score);
      return score != 0
          ? score
          : a.outcome.registration.adapterRef
              .compareTo(b.outcome.registration.adapterRef);
    });
    final candidates = ranked
        .map((item) => MuseRouteCandidateV2(
            adapterRef: item.outcome.registration.adapterRef,
            available: item.mode != null,
            effectiveModes: item.outcome.result?.effectiveModes
  ...
  MuseEngineMode? _mode(
      List<MuseEngineMode> modes, MuseRequestedMode requested, bool downgrade) {
