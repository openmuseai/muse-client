...
final class MuseAdapterRegistration {
  const MuseAdapterRegistration(
      {required this.adapter, required this.generation});
...
final class MuseAdapterRegistry {
  ...
  MuseAdapterLease register(MuseEngineAdapter adapter,
      {required int generation}) {
    if (generation < 1) throw ArgumentError.value(generation, 'generation');
    final registration =
        MuseAdapterRegistration(adapter: adapter, generation: generation);
  ...
  List<MuseAdapterRegistration> candidates(
    ...
    for (final registration in _registrations.values) {
      ...
      final supportsFormat =
          manifest.formats.any((format) => format.formatId == formatId);
      final supportsPlatform = manifest.platforms
          .any((platform) => platform.os == os && platform.arch == arch);
      final supportsPlacement = manifest.placements.any((item) =>
          item.name.replaceAllMapped(RegExp(r'(?<=[a-z0-9])([A-Z])'),
