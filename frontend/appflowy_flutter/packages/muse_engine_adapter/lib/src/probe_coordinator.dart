...
final class MuseProbeOutcome {
  const MuseProbeOutcome(
      {required this.registration, this.result, this.errorCode});
...
final class MuseProbeCoordinator {
  ...
  Future<List<MuseProbeOutcome>> probeAll(
    ...
    Future<void> worker() async {
      while (queue.isNotEmpty && !cancellation.isCancelled) {
        ...
        try {
          ...
          final result =
              await registration.adapter.probe(request, cancellation);
          cancellation.throwIfCancelled();
          ...
          outcomes.add(MuseProbeOutcome(
              registration: registration, errorCode: error.code));
        } catch (_) {
          outcomes.add(MuseProbeOutcome(
              registration: registration, errorCode: 'PROBE_FAILED'));
        }
      }
    }
    ...
    outcomes.sort((a, b) =>
        a.registration.adapterRef.compareTo(b.registration.adapterRef));
    return outcomes;
  }

  String _cacheKey(
      MuseAdapterRegistration registration, MuseProbeRequestV1 request) {
