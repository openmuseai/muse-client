...
void main() {
  ...
  setUpAll(() async {
    final fixtures = (jsonDecode(await File(
                '../../../../../../middlewares/dsh/core/contract-engine-session/fixtures/v1/messages.json')
            .readAsString()) as List)
        .cast<Map>();
    Map<String, Object?> value(String name) => (fixtures.singleWhere(
            (item) => item['name'].toString().startsWith(name))['value'] as Map)
        .cast<String, Object?>();
  ...
  test(
    ...
    final old =
        registry.register(_FakeAdapter(manifest, result), generation: 1);
  ...
  test('candidate matching uses verified format id and never extension hints',
      () {
    final registry = MuseAdapterRegistry()
  ...
  test('probe fan-out respects concurrency and cache dimensions', () async {
    ...
    final adapters = List.generate(5, (index) {
      final adapter = _FakeAdapter(manifest, result, onProbe: () async {
        calls++;
        active++;
    ...
    final coordinator =
        MuseProbeCoordinator(concurrency: 2, clock: () => 1789466400000);
    ...
    expect(cached.map((outcome) => outcome.result!.probeRef).toSet(),
        {'probe.cached'},
  ...
  test('router scores capability quality and makes downgrade explicit', () {
    ...
    final registration =
        MuseAdapterRegistration(adapter: adapter, generation: 1);
    final selection = const MuseEngineRouter().select(
        outcomes: [
          MuseProbeOutcome(
              registration: registration, result: null, errorCode: 'not used')
        ],
    ...
    final routed = const MuseEngineRouter().select(
        ...
        policy: const MuseRoutePolicy(
            revision: 'p', deniedAdapterRefs: {'muse.ioffice.word~1.0.0'}));
    expect(routed, isNull);
    final success = const MuseEngineRouter().select(
        outcomes: [
          MuseProbeOutcome(
              registration: registration, result: _ResultHolder.value)
        ],
        requestedMode: MuseRequestedMode.preferEdit,
        policy: const MuseRoutePolicy(revision: 'p'));
...
final class _FakeAdapter implements MuseEngineAdapter {
  ...
  Future<MuseProbeResultV1> probe(
      MuseProbeRequestV1 request, MuseCancellationToken cancellation) async {
    await onProbe?.call();
  ...
  @override
  Future<MuseEngineSessionHandleV1> open(MuseOpenEngineSessionRequestV1 request,
          MuseCancellationToken cancellation) =>
      throw UnimplementedError();
  @override
  Future<void> focus(MuseEngineSessionHandleV1 session) async {}
  @override
  Future<void> close(MuseEngineSessionHandleV1 session,
