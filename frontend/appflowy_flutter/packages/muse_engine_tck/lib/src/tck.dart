...
final class MuseAdapterTck {
  ...
  Future<MuseAdapterTckReport> run(MuseAdapterTckScenario scenario) async {
    ...
    if (manifest.formats.map((value) => value.formatId).toSet().length !=
        manifest.formats.length) failures.add('FORMAT_DUPLICATE');
    ...
    try {
      result =
          await scenario.adapter.probe(scenario.probeRequest, cancellation);
    } catch (_) {
    ...
    if (!manifest.materializations
        .toSet()
        .containsAll(result.materializationKinds))
