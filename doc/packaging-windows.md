...
Windows 分发对齐 macOS：安装（或解压）后即可运行，**不依赖本机 Node / 源码树**。DeepSeek sidecar 打进 `{exeDir}/muse/`，和 macOS `DSH Office.app/Contents/Resources/muse` 同一套运行时。
...
| 产物 | 用途 |
|---|---|
| `DSH Office/` | 便携目录，双击 `dsh-office.exe` 即可 |
| `DSH-Office-windows-x64.zip` | 给下载页的 zip（内含 `DSH Office/` 目录） |
| `DSH-Office-windows-x64-setup.exe` | 可选，Inno Setup 安装包（需本机安装 Inno Setup 6） |

用户只需：

1. 解压 zip，或运行 setup.exe（默认装到 `%LOCALAPPDATA%\Programs\DSH Office`）
2. 启动 **DSH Office**
3. 第一次打开 DeepSeek 面板时填写 `DEEPSEEK_API_KEY`

用户数据在 `%APPDATA%\DSH Office\Muse\`，不写进安装目录。
...
| [`install-windows-client.py`](../scripts/install-windows-client.py) | 拷到 `%LOCALAPPDATA%\Programs\DSH Office` 并建开始菜单 |
...
- `pack-macos-client.sh` → `pack-windows-client.py`
- `install-macos-client.sh` → `install-windows-client.py`
- `middlewares/scripts/stage-dsh-runtime.sh` → `muse_windows.stage_dsh_runtime`
...
- 打包 zip 不做代码签名。若要 SmartScreen 少拦，需用你们自己的 Authenticode 证书签 `dsh-office.exe` 和 setup.exe。
